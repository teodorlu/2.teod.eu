...
(defn d-index [published-ids draft-ids]
  (into (sorted-map)
        (map (fn [id]
               (let [path (dot-ttext->dot-html (str (d/find id)))]
                 [id (cond-> {:href (str "/" path) :text path}
                       (contains? draft-ids id) (assoc :draft true))])))
        (set/union published-ids draft-ids)))
...
(defn build [published-ids draft-ids]
  (let [published (into (sorted-set) (map (comp str d/find)) published-ids)
    ...
    (fs/create-dirs "js")
    (spit "js/index.mjs"
          (str "// Generated by bb build. Do not edit.\n\n"
               (js/export-const "d" (d-index published-ids draft-ids))))
